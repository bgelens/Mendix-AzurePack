...
if (([version](Get-CimInstance win32_operatingsystem).Version).Major -le 6) {
    $cert = New-SelfSignedCertificateEx -FriendlyName 'WinRM SSL' -Subject "CN=$env:COMPUTERNAME" -NotAfter ([datetime]::Today).AddYears(4) -StoreLocation LocalMachine -SignatureAlgorithm SHA256 -KeyUsage DigitalSignature,KeyEncipherment -EnhancedKeyUsage 'Server Authentication','Client Authentication'
} else {
...
if ($null -eq $cert) {
    throw 'No Certificate was created. Aborting!'
}
